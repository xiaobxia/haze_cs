...
<form id="pagerForm" onsubmit="return navTabSearch(this);"
      action="performance/toPerformancePage?myId=${params.myId}&groupLevel=8" method="post">
        ...
        <div class="searchBar">
            <table class="searchContent">
                <tr>
                    ...
                    <td>
                        手机号:
                        <input type="text" name="tel" id="tel_s3"
                               value="${params.tel }"/>
                    </td>

                    <td>派单时间：
                        <input type="text" name="begDate" id="begDate_s3" value="${params.begDate}"
                               class="date textInput readonly" datefmt="yyyy-MM-dd" readonly="readonly"/>
                        至 <input type="text" name="endDate" id="endDate_s3" value="${params.endDate}"
                                 class="date textInput readonly" datefmt="yyyy-MM-dd" readonly="readonly"/>
                    </td>

                    <td>
                        排序:
                        <select id="sortBy_s3" name="sortBy">
...
<script type="text/javascript">
    ...
    function reportExcel_s3(obj){
        var href=$(obj).attr("href");
        var begDate=$("#begDate_s3").val();
        var endDate=$("#endDate_s3").val();
        var tel = $("#tel_s3").val();
        var sortBy = $("#sortBy_s3").val();
        var toHref = href;
        if(begDate.length > 0 && toHref.indexOf(begDate) == -1) {
            toHref += "&begDate="+begDate;
        }
        if(endDate.length > 0 &&  toHref.indexOf(endDate) == -1) {
            toHref += "&endDate="+endDate;
        }
        if(tel.length > 0 && toHref.indexOf(tel) == -1) {
            toHref += "&tel="+tel;
        }
