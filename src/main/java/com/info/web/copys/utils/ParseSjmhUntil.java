package com.info.web.copys.utils;
import com.google.gson.*;
import com.info.risk.pojo.Advice;
import com.info.risk.pojo.Reason;
import com.info.risk.pojo.Supplier;
import com.info.web.copys.pojo.CallInfo;
import com.info.web.copys.pojo.NumberCallInfo;
import com.info.web.copys.pojo.ShuJuMoHeVo;
import com.info.web.copys.pojo.TdTopTen;
...
//        adviceList.add(adviceTerm3(accountBalance));
        ...
        for (String s : monthList) {
            ...
            if ("".equals(monthNightFrequencyInit)) {
...
//        }
        ...
        if (Advice.REJECT.equals(adviceTerm41(person, sameRate))) {
            shuJuMoHeVo.getAutoList().add("撞库率低于20%（通讯录人数30-100）");
            shuJuMoHeVo.getAdviceList().add(Advice.REJECT);
        }
        if (Advice.REJECT.equals(adviceTerm42(person, sameRate))) {
            shuJuMoHeVo.getAutoList().add("撞库率低于17%（通讯录人数100以上）");
            shuJuMoHeVo.getAdviceList().add(Advice.REJECT);
        }
    ...
    private boolean getNightFlag(String time) {
        ...
        if (hhmm.compareTo("06:00") < 0 || hhmm.compareTo("21:00") > 0) {
            return true;
        }
        return false;
    }
